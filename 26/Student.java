...
public class Student {
    ...
    public String toString() {
        return "Student[id=%d, %s, %s, age=%d, enrolled=%d, active=%d months ago]"
                .formatted(studentId, countryCode, gender, getAge(),
                           yearEnrolled, getMonthsSinceActive());
    }

    public static Student getRandomStudent(Course... courses) {
        long id        = nextId++;
        String country = COUNTRIES[random.nextInt(COUNTRIES.length)];
        int year       = 2022 + random.nextInt(4);   // enrolled in the past 4 years: 2022-2025
        ...
        // randomly select 1 to N courses (at least one)
        List<Course> courseList = new ArrayList<>(Arrays.asList(courses));
        Collections.shuffle(courseList, random);
        int numCourses = 1 + random.nextInt(courses.length);

        for (int i = 0; i < numCourses; i++) {
            Course course = courseList.get(i);

            int enrollYear  = 2022 + random.nextInt(4);
            ...
            // minimum lecture number is 30
            int lecture = 30 + random.nextInt(Math.max(1, course.getLectureCount() - 29));
